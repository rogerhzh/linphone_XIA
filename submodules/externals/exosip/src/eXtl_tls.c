...
static int check_names(const char *value, const char *CN)
{
	int lenc=strlen(CN);
...
// From http://stackoverflow.com/questions/15875494/how-to-retrieve-issuer-alternative-name-for-ssl-certificate-by-openssl
// http://stackoverflow.com/questions/15875494/how-to-retrieve-issuer-alternative-name-for-ssl-certificate-by-openssl
// License MIT
typedef enum {
	HVRMatchFound,
	HVRMatchNotFound,
	HVRNoSANPresent,
	HVRMalformedCertificate
} HostnameValidationResult;
static int matches_subject_alternative_name (const char *hostname, const X509 *server_cert) {
	HostnameValidationResult result = HVRMatchNotFound;
	int i;
	int san_names_nb = -1;
	...
	for (i=0; i<san_names_nb; i++) {
		...
		if (current_name->type == GEN_DNS) {
			...
			if (ASN1_STRING_length(current_name->d.dNSName) != strlen(dns_name)) {
				result = HVRMalformedCertificate;
				break;
			}
			else { // Compare expected hostname with the DNS name
                if (check_names(hostname, dns_name) == 0) {
...
static int _tls_tl_ssl_connect_socket(struct socket_tab *sockinfo)
{
	...
	if (cert != 0) {
		...
		if (tls_verify_client_certificate && tls_verify_client_cn){
			HostnameValidationResult alternateNameValidation;
			char peer_CN[256];
			memset(peer_CN, 0, sizeof(peer_CN));
			X509_NAME_get_text_by_NID(X509_get_subject_name(cert), NID_commonName,
									  peer_CN, sizeof(peer_CN));

			alternateNameValidation = matches_subject_alternative_name(sockinfo->canonical_host, cert);
			if (alternateNameValidation == HVRMalformedCertificate) {
				SSL_set_verify_result(sockinfo->ssl_conn,
						      X509_V_ERR_APPLICATION_VERIFICATION + 1);
				X509_free(cert);
				OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL,
						      "Maflormed certificate detected during alternate name detection for %s\n", sockinfo->canonical_host));
				return -1;
			}
			if (alternateNameValidation != HVRMatchFound && 0!=check_names(sockinfo->canonical_host, peer_CN)) {
				SSL_set_verify_result(sockinfo->ssl_conn,
									  X509_V_ERR_APPLICATION_VERIFICATION + 1);
				X509_free(cert);
				OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL,
